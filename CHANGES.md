...
- **Estado**: `[x]` completado
- **Scope**:
  - Catálogo administrable: tablas `Role` (permissions via JSONB), `usuario_role` (junction) con diseño alternativo a normalized Permiso+RolPermiso (ver D1 en design.md).
  - Roles del dominio seed: ALUMNO, TUTOR, PROFESOR, COORDINADOR, NEXO, ADMIN, FINANZAS.
  - Resolución de permisos efectivos server-side por request (unión de roles via frozenset, acotada por tenant).
  - Dependency/guard `require_permission("modulo:accion")` que declara el permiso por endpoint; sin él → 403.
  - `Migración 003: role, usuario_role` + seed de la matriz base de `03_actores_y_roles.md` §3.3.
  - Tests: usuario sin permiso → 403, unión de roles, catálogo administrable, role CRUD, user-role assignment.
- **Dependencias**: `C-03`
- **Governance**: CRITICO
- **Leer antes**:
